...
export default function ResultsPage() {
  ...
  const overallScore = result.eco_score?.overall_score || result.score?.total || 0;
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-800">Your EcoBee Results</h1>
              ...
              <div className="text-center">
                <div className="relative inline-block mb-4">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center flex-col">
                    ...
                    <div className={`text-lg font-bold px-3 py-1 rounded-full mt-1 ${getScoreBg(overallScore)}`}>
                      {grade}
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { 
                    name: "Food Choices", 
                    score: result.eco_score?.category_scores?.meal || result.score?.breakdown?.food_choices || 0,
                    icon: "🍽️",
                    color: "green"
                  },
                  { 
                    name: "Transportation", 
                    score: result.eco_score?.category_scores?.transportation || result.score?.breakdown?.transportation || 0,
                    icon: "🚗",
                    color: "blue"
                  },
                  { 
                    name: "Daily Actions", 
                    score: result.eco_score?.category_scores?.environmental_actions || result.score?.breakdown?.daily_actions || 0,
                    icon: "♻️",
                    color: "purple"
                  },
                  { 
                    name: "Clothing", 
                    score: result.eco_score?.category_scores?.clothing || result.score?.breakdown?.clothing || 0,
                    icon: "👕",
                    color: "yellow"
                  }
                ].map((category, index) => (
                  <div key={index} className={`p-3 rounded-lg border-2 ${getScoreBg(category.score)}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 mt-4">
              ...
              <div className="space-y-3">
                {result.analysis.reflection_analysis.strengths?.length > 0 && (
                  <div className="bg-green-50 p-3 rounded-lg">
                    ...
                    <ul className="space-y-1">
                      {result.analysis.reflection_analysis.strengths.slice(0, 2).map((strength: string, index: number) => (
                        <li key={index} className="text-green-700 text-xs flex items-start">
                ...
                {result.analysis.reflection_analysis.suggestions?.length > 0 && (
                  <div className="bg-amber-50 p-3 rounded-lg">
                    <h4 className="font-semibold text-amber-800 text-sm mb-2">Suggestions</h4>
                    <ul className="space-y-1">
                      {result.analysis.reflection_analysis.suggestions.slice(0, 2).map((suggestion: string, index: number) => (
                        <li key={index} className="text-amber-700 text-xs flex items-start">
                          <span className="mr-1">💡</span>
                          {suggestion}
                        </li>
                      ))}
            ...
            {result.analysis.reflection_analysis.themes?.length > 0 && (
              <div className="mt-4 pt-4 border-t border-gray-200">
                <h4 className="font-semibold text-gray-700 text-sm mb-2">Key Themes</h4>
                <div className="flex flex-wrap gap-2">
                  {result.analysis.reflection_analysis.themes.slice(0, 5).map((theme: string, index: number) => (
        ...
        {(result.analysis || result.comprehensive_analysis) && (
          <div className="bg-white rounded-xl shadow-lg p-6 mt-4">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {/* Impact Summary */}
              {(result.analysis?.impact_summary || result.comprehensive_analysis?.overall_impact) && (
                <div className="bg-green-50 p-4 rounded-lg">
                  <h4 className="font-semibold text-green-800 mb-2">Today's Impact</h4>
                  <p className="text-green-700 text-sm">
                    {result.analysis?.impact_summary || 
                     `${result.comprehensive_analysis?.overall_impact?.environmental_actions_taken || 0} environmental actions taken`}
                  </p>
                </div>
              )}

              {/* Top Recommendations */}
              {(result.analysis?.recommendations || result.comprehensive_analysis?.recommendations)?.length > 0 && (
                <div className="bg-blue-50 p-4 rounded-lg">
                  <h4 className="font-semibold text-blue-800 mb-2">Top Recommendations</h4>
                  <ul className="space-y-1">
                    {(result.analysis?.recommendations || result.comprehensive_analysis?.recommendations || [])
